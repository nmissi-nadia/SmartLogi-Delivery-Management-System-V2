 ...
 */
    ...
    public Page<ColisDTO> findAll(String statut, String ville, String priorite, String zoneId, 
        ...
        if (authorities.stream().anyMatch(a -> a.getAuthority().equals("ROLE_LIVREUR"))) {
            livreurId = livreurRepository.findByUserUsername(username).map(Livreur::getId).orElse(null);
        } else if (authorities.stream().anyMatch(a -> a.getAuthority().equals("ROLE_CLIENT"))) {
            clientId = clientExpediteurRepository.findByUserUsername(username).map(ClientExpediteur::getId).orElse(null);
        }
    ...
    public boolean isLivreurOfColis(String colisId, String username) {
        Colis colis = colisRepository.findById(colisId).orElse(null);
        return colis != null && colis.getLivreur() != null && colis.getLivreur().getUser().getUsername().equals(username);
    }

    public boolean isClientOfColis(String colisId, String username) {
        Colis colis = colisRepository.findById(colisId).orElse(null);
        return colis != null && colis.getClientExpediteur() != null && colis.getClientExpediteur().getUser().getUsername().equals(username);
    }
}